...
function ResetPasswordForm() {
  ...
  // We need to wrap this in Suspense because it uses useSearchParams
  const searchParams = useSearchParams();
  const token = searchParams.get("token"); // Handle 'token' param if present
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-zinc-300">Confirmer le mot de passe</FormLabel>
                  <FormControl>
                    <Input 
                      type="password" 
                      placeholder="••••••••" 
            ...
            <Button type="submit" className="w-full bg-white text-black hover:bg-zinc-200" disabled={isLoading}>
             ...
             <Button asChild variant="link" className="w-full font-normal text-zinc-400 hover:text-white">
              <Link href="/sign-in">
